# ─────────────────────────────────────────────────────────────────────────────
# Spanish Data Protection MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t spanish-data-protection-mcp .
# Run:    docker run --rm -p 3000:3000 spanish-data-protection-mcp
#
# The image bakes /app/data/aepd.db at build time. Override AEPD_DB_PATH
...
ENV AEPD_DB_PATH=/app/data/aepd.db
...
# Bake the pre-built database into the image so /app/data/aepd.db resolves
# at runtime without a bind mount. This matches the working pattern in
# cypriot-data-protection-mcp's GHCR image.
#
# The explicit `data/aepd.db` reference is required — `.github/workflows/
...
# We then COPY it into the image at /app/data/aepd.db (AEPD_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/aepd.db
